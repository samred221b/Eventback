...
const CACHE_EXPIRY_TIME = 15 * 60 * 1000; // 15 minutes (increased for better performance)
...
const cacheOrganizerData = async (organizerId, organizerData, eventsData) => {
  try {
    ...
    logger.log('Cached organizer data:', { 
      organizerId, 
...
const clearOrganizerCache = async (organizerId) => {
  try {
    ...
    logger.log('Cleared cache for organizer:', organizerId);
  } catch (error) {
    logger.warn('Failed to clear organizer cache:', error);
  }
};
...
const isCacheValid = (cacheData) => {
  ...
  if (Date.now() - cacheData.timestamp > CACHE_EXPIRY_TIME) {
    logger.log('Cache expired, clearing cache');
    return false;
  }
...
const getCachedOrganizerData = async (organizerId) => {
  try {
    ...
    if (!isCacheValid(parsedData)) {
      await clearOrganizerCache(organizerId);
      return null;
    }
    
    logger.log('Using cached data for organizer:', organizerId);
    return { 
...
const refreshCacheInBackground = async (organizerId) => {
  try {
    logger.log('Background cache refresh for organizer:', organizerId);
    ...
    try {
      ...
      const filteredEvents = eventsData.filter(event => {
        const eventOrganizerId = event.organizerId?._id || event.organizerId?.id || event.organizerId;
        return eventOrganizerId === organizerId;
      });
      
      // Update cache in background
      await cacheOrganizerData(organizerId, organizerData, filteredEvents);
      logger.log('Background cache refresh completed for organizer:', organizerId);
    } catch (eventsError) {
      logger.warn('Background events refresh failed:', eventsError);
    }
  } catch (error) {
...
function Organprofilescreenforusers({ route, navigation }) {
  ...
  useEffect(() => {
    if (organizerId) {
      ...
      const backgroundRefreshInterval = setInterval(() => {
        refreshCacheInBackground(organizerId);
  ...
  const onRefresh = async () => {
    ...
    await clearOrganizerCache(organizerId); // Clear cache for debugging
  ...
  const fetchOrganizerProfile = async (forceRefresh = false) => {
    try {
      // Debug: Log the organizerId being used
      logger.log('Fetching organizer profile for ID:', organizerId);
      ...
      if (!forceRefresh) {
        ...
        if (cachedData) {
          ...
          if (cacheAge > 5 * 60 * 1000) { // 5 minutes
            setTimeout(() => refreshCacheInBackground(organizerId), 1000);
          }
      ...
      let filteredEvents = [];
      try {
        // Try different API endpoint approaches
        let eventsResponse;
        
        // First try the standard approach
        try {
          eventsResponse = await apiService.get(`/events?organizerId=${organizerId}&limit=10`);
        } catch (firstError) {
          // Try alternative approach
          eventsResponse = await apiService.get(`/events/organizer/${organizerId}?limit=10`);
        }
        
        eventsData = eventsResponse.data.success ? eventsResponse.data.data : eventsResponse.data;
        
        // Debug: Log the response to see what we're getting
        logger.log('Events response for organizer:', organizerId, {
          totalEvents: eventsData.length,
          eventIds: eventsData.map(e => ({ id: e._id, organizerId: e.organizerId, organizerIdField: e.organizerId }))
        });
        
        // Additional filter to ensure we only show events by this organizer
        filteredEvents = eventsData.filter(event => {
          const eventOrganizerId = event.organizerId?._id || event.organizerId?.id || event.organizerId;
          return eventOrganizerId === organizerId;
        });
        
        logger.log('Filtered events:', {
          before: eventsData.length,
      ...
      // Cache the data
      await cacheOrganizerData(organizerId, organizerData, filteredEvents.length > 0 ? filteredEvents : eventsData);
        ...
        >
          ...
          {isFromCache && !loading && (
            <View style={{
              ...
              <Feather name="info" size={14} color="#0277BD" />
              <Text style={{
                marginLeft: 6,
                fontSize: 12,
                color: '#0277BD',
